...
# Stage 2 — ball tracking + spatial filtering (SORT-style Kalman + Hungarian)
.venv/bin/python track.py -i <video>_detected.json

# Stage 3 — render annotated video
.venv/bin/python render.py -i <video> -j <video>_tracked.json -o <output>.mp4
```
...
- **Unified JSON format:** all stages share the same COCO schema. track.py adds `track_id` (int|null). render.py reads any stage; `track_id` defaults to null and `valid` defaults to true when absent.
...
- **track.py output** contains only valid detections after spatial filtering (invalid ones are discarded).
...
| `track.py` | Stage 2: tracking + spatial filtering → COCO JSON with track_id |
...
| `extract_pos_balls.py` | Extract ball sample frames as JPEG + export sports ball annotations to training COCO JSON |

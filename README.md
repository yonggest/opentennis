...
网球比赛视频分析流水线：检测球员、球拍、网球，检测球场关键点，追踪网球，过滤无效检测，输出标注视频。
...
.venv/bin/python detect.py \
  -i <video> \
  -m models/yolo26x.pt \
  -s models/yolo26n-seg-tuned.pt \
  -z 1920
```

输出：`<video>_detected.json`（COCO 格式，含球场关键点和缓冲区凸包）
...
输出：`<video>_tracked.json`（同 COCO 格式，每个检测标注加 `track_id` 字段；网球 gap 帧线性插值补全；球员/球拍平滑脚步/中心点写入 `foot`/`center` 字段）
...
输出：`<video>_parsed.json`（同 COCO 格式，每个标注加 `valid` 字段）
...
`-j` 可传任意阶段的 JSON（`track_id` 缺省为 null，`valid` 缺省为 true）。输出：H.264 标注视频（crf=18）。
...
可视化任意阶段的 JSON，支持缩放/平移/播放，显示轨迹预测线和搜索圆，无效标注用暗色 + X 显示。
...
各阶段共用同一 COCO 格式，向后兼容：

```
images       : [{id, width, height, frame_id}, ...]
annotations  : [{id, image_id, category_id, bbox [x,y,w,h], area, iscrowd, score,
                 track_id?,      ← track.py 追加（int | null）
                 interpolated?,  ← track.py gap 插值帧标记（bool，仅网球）
                 foot?,          ← track.py 追加（[x,y]，球员高斯平滑脚步位置）
                 center?,        ← track.py 追加（[x,y]，球拍高斯平滑中心位置）
                 valid?},        ← parse.py 追加（bool，缺省视为 true）
                ...]
...
## 追踪算法（track.py / tracker.py）

三类目标共用同一套两阶段 ByteTrack 风格框架（高置信度先匹配，低置信度只续接已有轨迹）+ 匈牙利算法，但各类目标参数和后处理不同。

### 网球追踪（BallTracker）

**预过滤**：每帧球检测先按 bbox 面积和宽高比过滤，排除过大/过小/过扁的误检。面积范围和宽高比阈值由帧率和像素/米比例从物理常量推算（`BallTracker.from_video`）。

**搜索门限**（逐轨迹动态，与透视缩放自适应）：
- 轨迹仅有 1 个历史点（无方向信息）→ `max_dist`（单帧最大球速像素位移，物理兜底）
- 轨迹有 2+ 个历史点 → `search_diameters × ball_d_px`（最近一帧检测 bbox 均值宽高）

**Gap 插值**：相邻两段检测之间的空缺帧用线性插值补全（conf=0，`interpolated=True`），TENTATIVE 阶段的检测回填到对应的 CONFIRMED 轨迹头部。

### 球员追踪（PlayerTracker）

以 bbox **底部中心（脚点）**为追踪锚点，搜索门限基于球员最大冲刺速度（8 m/s）换算的像素位移，不使用线性预测（遮挡时停在上次位置等待重现）。颜色直方图（球衣上半区 HSV-H）作为外观代价调制匹配分数，防止左右球员 ID 互换。追踪后对每条轨迹的脚步位置做高斯平滑（σ = 0.1 s），结果写入 `det['foot']`。

### 球拍追踪（RacketTracker）

以 bbox **中心**为追踪锚点，搜索门限基于球拍最大速度（12 m/s，含球员移动 + 挥拍）换算的像素位移，不使用线性预测。追踪后对每条轨迹的中心位置做高斯平滑（σ = 0.1 s），结果写入 `det['center']`。
...
用 `yolo26n-seg-tuned.pt` 对第一帧做实例分割，得到球场区域多边形。对凸包做近似，提取四个角点，用 `getPerspectiveTransform` 计算初始 H。这一步给出足够准确的起点，使后续优化不会陷入错误局部极小。
...
`parse.py` 对三类目标分别过滤，过滤结果写入 `valid` 字段（保留原始检测，便于调试）。

### 球员过滤

用 **ground_hull**（地面缓冲区四边形）。球员 bbox **底部中心**在 ground_hull 内 → 有效。

### 球拍过滤

用 **volume_hull**（立体缓冲区凸包）。bbox 的中心点或任意一个角点（共 5 点）在 volume_hull 内 → 有效。
...
| 条件 | 类型 |
|---|---|
| 轨迹全局包围盒对角线 < 20 px，或单帧孤立检测 | 静止球 |
| 轨迹全局包围盒对角线 ≥ 20 px | 运动球 |

**步骤 2 — 按类型过滤**

| 类型 | 规则 |
|---|---|
| 运动球 | 轨迹**起点**落在左墙或右墙四边形内 → 整条轨迹无效 |
| 静止球 | 每帧逐点检测，该点在 volume_hull 内 → 有效 |

**侧边墙四边形**：由 `vol_bottom_pts`（底面四角）和 `vol_top_pts`（顶面四角）的左右两侧各取两个角点，沿底→顶方向延伸到图像顶部以上，形成两个梯形四边形。用墙面四边形而非立体凸包判断起点，可避免对高抛球/发球的误判（立体凸包受 2 m 高度限制）。

### 侧边墙遮罩（可视化）

render.py 和 browse.py 在侧边墙外部绘制半透明暗色遮罩，边界沿墙竖边方向（底面角 → 顶面角 → 天空）延伸，视觉上呈现为从地面延伸到天空的墙体阴影。
...
| `models/yolo26n-seg-tuned.pt` | 球场区域分割，用于初始化单应矩阵（YOLO26n-seg fine-tuned） |

### NMS

`detect.py` 禁用 YOLO 内置 NMS（`iou=1.0`），由 `objects_detector.py` 对三类目标分别做贪心 NMS，阈值各异：

| 类别 | IoU 阈值 | 理由 |
|---|---|---|
| person | 0.45 | 体型稳定，标准值 |
| tennis racket | 0.35 | 形变大，宁可少保留 |
| sports ball | 0.65 | 目标小，避免误删 |
...
```bash
.venv/bin/python train_segment.py \
...
```bash
cp runs/segment/<run_name>/weights/best.pt models/yolo26n-seg-tuned.pt
```
